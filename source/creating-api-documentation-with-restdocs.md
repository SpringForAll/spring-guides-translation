...
针对于上述测试（mock）HTTP 请求和断言响应，所创建的 HTTP API 可以生成动态的内容，因为它能够窥探测试并清除 HTTP 请求，以便在文档中使用。Spring REST 文档允许你通过生成“片段”来实现。你可以很容易地得到它，只需要将“注解”和“断言”添加到你的测试用例中，以下是一个完整的测试：
